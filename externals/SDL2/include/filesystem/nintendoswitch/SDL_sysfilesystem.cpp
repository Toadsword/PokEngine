...
{
    nn::account::UserHandle *handle = SDL_NintendoSwitch_UserHandle;
    if (!handle) {
        handle = (nn::account::UserHandle *) SDL_NintendoSwitch_GetPreselectedUserHandle();
        if (handle == NULL) {
            return NULL;
        }

        nn::account::Uid uid;
        nn::Result rc = nn::account::GetUserId(&uid, *handle);
        if (rc.IsFailure()) {
...
void *
SDL_NintendoSwitch_GetPreselectedUserHandle(void)
{
    if (!SDL_NintendoSwitch_UserHandle) {
        ...
        if (!nn::account::TryOpenPreselectedUser(handle)) {
            SDL_SetError("Couldn't open preselected user account. Is your NMETA file configured to have the system select a user on startup?");
            delete handle;
            return NULL;
        }
    ...
    return SDL_NintendoSwitch_UserHandle;
}
